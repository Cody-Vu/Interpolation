...
namespace cartesian_impedance_controller
{
  ...
  void CartesianImpedanceControllerRos::trajUpdate()
  {
    ...
    ros::Duration time_from_start = ros::Time::now() - this->traj_start_;
    
    // Find the two waypoints that bracket the current time
    size_t next_index = this->traj_index_;
    while (next_index < trajectory_.points.size() && 
           time_from_start > trajectory_.points.at(next_index).time_from_start)
    {
      next_index++;
    }
    
    // Update index for next time
    this->traj_index_ = next_index;
    
    // If we're at the last point, just use that point
    if (next_index >= trajectory_.points.size())
    {
      ...
      // Set nullspace configuration to the last point
      Eigen::VectorXd q = Eigen::VectorXd::Map(trajectory_.points.back().positions.data(),
                                             trajectory_.points.back().positions.size());
      this->setNullspaceConfig(q);
      return;
    }
    
    // If we're at the first point, just use that point
    if (next_index == 0)
    {
      this->position_d_target_ = this->traj_positions_[0];
      this->orientation_d_target_ = this->traj_orientations_[0];
      
      // Set nullspace configuration to the first point
      Eigen::VectorXd q = Eigen::VectorXd::Map(trajectory_.points[0].positions.data(),
